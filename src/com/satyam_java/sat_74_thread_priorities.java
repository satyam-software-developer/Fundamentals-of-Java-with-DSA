...
//        The thread scheduler is responsible for assigning the processor to a thread.
//        But the question is on what basis the thread scheduler decides that a particular thread will run before other threads.
...
//        The programmer can explicitly assign the priority to the thread.
//        Otherwise, JVM automatically assigns priority while creating the thread.
//        In Java, we can specify the priority of each thread relative to other threads.
//        Those threads having higher priorities get greater access to the available resources than lower priorities threads.
...
//        IllegalArgumentException is thrown if the priority given by the user is out of the range [1,10].
//        Syntax :
...
public class sat_74_thread_priorities {
    public static void main(String[] args) {
